...
class FoglalasKezelo:
    ...
    def foglalas(self, datum_str, szoba_tipusa):
        ...
        for szoba in self.szalloda.szobak:
            if szoba.szoba_tipusa() != szoba_tipusa:
                continue
            foglalt = any(
                foglalas.szobaszam == szoba.szobaszam and foglalas.datum == datum for foglalas in self.foglalasok)
            if not foglalt:
                foglalas = Foglalas(szoba.szobaszam, datum, szoba.ar)
                self.foglalasok.append(foglalas)
                print(
                    f"Foglalás sikeresen létrehozva: Szobaszám {szoba.szobaszam}, Dátum: {datum_str}, Ár: {szoba.ar} Ft")
                return
    ...
    def foglalasok_listazasa(self):
        ...
        for foglalas in self.foglalasok:
            print(
                f"Szobaszám: {foglalas.szobaszam}, Dátum: {foglalas.datum.strftime('%Y-%m-%d')}, Ár: {foglalas.ar} Ft")
